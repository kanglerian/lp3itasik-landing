<div class="flex-1 overflow-x-auto">
  <div class="space-y-2 mb-2 pl-2">
    ...
    <p class="text-gray-500 text-sm">Fitur ini memungkinkan Anda untuk menambah, menghapus, dan mengedit program studi di Politeknik LP3I.</p>
  </div>
  <form id="form" action="<?= base_url() ?>program/insert" class="flex flex-col md:flex-row md:items-start gap-4 py-3 px-2" method="post" enctype="multipart/form-data">
    <div class="flex-1">
      <div class="flex flex-col gap-2">
        <input type="text" name="title" id="title" class="w-full p-2 text-gray-700 border border-gray-300 rounded-md bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500" placeholder="Type name major.." required>
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        ...
        <?php if (!empty($programs)) { ?>
          <?php foreach ($programs as $number => $program) : ?>
            <tr class="bg-white border-b transition ease-in-out duration-200 hover:bg-gray-50">
              ...
              <td colspan="2" class="space-y-2 px-6 py-4">
                <!-- Toggle -->
                <a role="button" href="<?= base_url() ?>program/change/<?= $program->id ?>" class="block md:inline text-center text-white px-2 py-1 text-sm rounded <?= $program->status == "1" ? 'bg-blue-600' : 'bg-red-600' ?>"><?= $program->status == "1" ? '<i class="fa-solid fa-toggle-on fa-1x"></i>' : '<i class="fa-solid fa-toggle-off fa-1x"></i>' ?></a>
                <!-- Edit -->
                <a role="button" onclick="editInformation(`<?= $program->id ?>`,`<?= $program->title ?>`,`<?= $program->level ?>`,`<?= $program->campus ?>`,`<?= $program->description ?>`,`<?= $program->status ?>`)" class="block md:inline text-center bg-amber-400 px-2 py-1 text-sm rounded text-white"><i class="fa-regular fa-pen-to-square"></i></a>
                <!-- Delete -->
                <a role="button" data-modal-target="popup-modal<?= $program->id ?>" data-modal-toggle="popup-modal<?= $program->id ?>" class="block md:inline text-center bg-red-600 px-2 py-1 text-sm rounded text-white"><i class="fa-solid fa-trash"></i></a>
                <div id="popup-modal<?= $program->id ?>" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                  <div class="relative w-full h-full max-w-md md:h-auto">
                    <div class="relative bg-white rounded-lg shadow">
                      <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="popup-modal<?= $program->id ?>">
                        <i class="fa-solid fa-xmark"></i>
                      </button>
                      <div class="p-6 text-center">
                        <i class="block mb-5 text-gray-500 fa-solid fa-circle-exclamation fa-3x"></i>
                        <h3 class="mb-5 text-lg font-normal text-gray-500">Kamu yakin akan menghapus <?= $program->title ?>?</h3>
                        <a href="<?= base_url() ?>program/delete/<?= $program->id ?>" role="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                          Ya, tentu saja!
                        </a>
                        <button data-modal-hide="popup-modal<?= $program->id ?>" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">Tidak, batalkan</button>
                      </div>
                    </div>
                  </div>
                </div>
...
<script>
  const editInformation = (id, title, level, campus, description, status) => {
    let titleInput = document.getElementById('title');
    let levelInput = document.getElementById('level');
    let campusInput = document.getElementById('campus');
    ...
    form.setAttribute("action", `<?= base_url() ?>program/update/${id}`);
