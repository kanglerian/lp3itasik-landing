<div class="flex-1 overflow-x-auto">
  <div class="space-y-2 mb-2 pl-2">
    ...
    <p class="text-gray-500 text-sm">Fitur dokumentasi adalah daftar foto kegiatan yang dilaksanakan di LP3I.</p>
  </div>
  <form id="form" action="<?= base_url() ?>documentation/insert" class="flex flex-col md:flex-row md:items-start gap-4 py-3 px-2" method="post" enctype="multipart/form-data">
    <div class="flex-1">
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        ...
        <?php if (!empty($documentations)) { ?>
          <?php foreach ($documentations as $number => $documentation) : ?>
            <tr class="bg-white border-b transition ease-in-out duration-200 hover:bg-gray-50">
              <td class="px-6 py-4">
                <?= $number + 1 ?>
              </td>
              <th scope="row" class="px-6 py-4">
                <img src="<?= base_url() ?>uploads/<?= $documentation->image ?>" class="w-44 rounded">
              </th>
                ...
                <div id="popup-modal<?= $documentation->id ?>" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                  <div class="relative w-full h-full max-w-md md:h-auto">
                    <div class="relative bg-white rounded-lg shadow">
                      <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="popup-modal<?= $documentation->id ?>">
                        <i class="fa-solid fa-xmark"></i>
                      </button>
                      <div class="p-6 text-center">
                        <i class="block mb-5 text-gray-500 fa-solid fa-circle-exclamation fa-3x"></i>
                        <h3 class="mb-5 text-lg font-normal text-gray-500">Kamu yakin akan menghapus <?= $documentation->title ?>?</h3>
                        <a href="<?= base_url() ?>documentation/delete/<?= $documentation->id ?>" role="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                          Ya, tentu saja!
                        </a>
                        <button data-modal-hide="popup-modal<?= $documentation->id ?>" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">Tidak, batalkan</button>
                      </div>
                    </div>
                  </div>
                </div>
...
<script>
  const editInformation = (id, title, status) => {
    ...
    form.setAttribute("action", `<?= base_url() ?>documentation/update/${id}`);
