<div class="flex-1 overflow-x-auto">
  <div class="space-y-2 mb-2 pl-2">
    ...
    <p class="text-gray-500 text-sm">Fitur ini memungkinkan Anda untuk menambah, menghapus, dan mengedit keunggulan kuliah di Politeknik LP3I Kampus Tasikmalaya.</p>
  </div>
  <form id="form" action="<?= base_url() ?>benefit/insert" class="flex flex-col md:flex-row md:items-start gap-4 py-3 px-2" method="post" enctype="multipart/form-data">
    <div class="flex-1">
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        ...
        <?php if (!empty($benefits)) { ?>
          <?php foreach ($benefits as $number => $benefit) : ?>
            <tr class="bg-white border-b transition ease-in-out duration-200 hover:bg-gray-50">
              <td class="px-6 py-4">
                <?= $number + 1 ?>
              </td>
              <th scope="row" class="px-6 py-4">
                <img src="<?= base_url() ?>uploads/<?= $benefit->image ?>" class="w-44 rounded-2xl">
              </th>
              <th scope="row" class="px-6 py-4 font-medium text-gray-900">
                <?= $benefit->title ?><br>
                <p class="text-gray-500 font-normal text-xs mt-2 text-justify"><?= $benefit->description ?></p>
              </th>
              <td colspan="2" class="space-y-2 px-6 py-4">
                <!-- Toggle -->
                <a role="button" href="<?= base_url() ?>benefit/change/<?= $benefit->id ?>" class="block md:inline text-center text-white px-2 py-1 text-sm rounded <?= $benefit->status == "1" ? 'bg-blue-600' : 'bg-red-600' ?>"><?= $benefit->status == "1" ? '<i class="fa-solid fa-toggle-on fa-1x"></i>' : '<i class="fa-solid fa-toggle-off fa-1x"></i>' ?></a>
                <!-- Edit -->
                <a role="button" onclick="editInformation('<?= $benefit->id ?>','<?= $benefit->title ?>','<?= $benefit->description ?>','<?= $benefit->status ?>')" class="block md:inline text-center bg-amber-400 px-2 py-1 text-sm rounded text-white"><i class="fa-regular fa-pen-to-square"></i></a>
                <!-- Delete -->
                <a role="button" data-modal-target="popup-modal<?= $benefit->id ?>" data-modal-toggle="popup-modal<?= $benefit->id ?>" class="block md:inline text-center bg-red-600 px-2 py-1 text-sm rounded text-white"><i class="fa-solid fa-trash"></i></a>
                <div id="popup-modal<?= $benefit->id ?>" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                  <div class="relative w-full h-full max-w-md md:h-auto">
                    <div class="relative bg-white rounded-lg shadow">
                      <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="popup-modal<?= $benefit->id ?>">
                        <i class="fa-solid fa-xmark"></i>
                      </button>
                      <div class="p-6 text-center">
                        <i class="block mb-5 text-gray-500 fa-solid fa-circle-exclamation fa-3x"></i>
                        <h3 class="mb-5 text-lg font-normal text-gray-500">Kamu yakin akan menghapus <?= $benefit->title ?>?</h3>
                        <a href="<?= base_url() ?>benefit/delete/<?= $benefit->id ?>" role="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                          Ya, tentu saja!
                        </a>
                        <button data-modal-hide="popup-modal<?= $benefit->id ?>" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">Tidak, batalkan</button>
                      </div>
                    </div>
                  </div>
                </div>
...
<script>
  const editInformation = (id, title, description, status) => {
    ...
    form.setAttribute("action", `<?= base_url() ?>benefit/update/${id}`);
