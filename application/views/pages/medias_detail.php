		...
		<div class="flex flex-col justify-end md:flex-row gap-5">
					...
					<ul class="space-y-3">
						<?php if (!empty($medias)) { ?>
							<?php foreach ($medias as $news) { ?>
								<li>
									<a href="<?= base_url() ?>blogs/media/<?= $news->uuid ?>" class="bg-gray-100 py-1 px-2 rounded text-base text-sky-600 underline md:hover:text-sky-700"><?= $news->title ?></a>
								</li>
							<?php } ?>
						<?php } else { ?>
			...
			<div class="w-full md:w-2/3">
				...
				<div class="space-y-5 mt-4 bg-gray-100 p-5 rounded-xl">
					...
					<span class="inline-block bg-sky-200 text-sky-700 text-sm py-1 px-5 rounded-md mb-3"><i class="fa-solid fa-cmediandar-days mr-2"></i> <?= date("d F Y", strtotime($media->date)) ?></span>
